...
PS.ui.bookmarks = (function() {
  var maxBookmarks = 100;
  var maxLabelLength = 64;
  var maxNoteLength = 240;
  ...
  function copyEventTarget(event, source) {
    if (!event) {
      return null;
    }
    ...
    return {
      ...
      label: cleanText(event.label || "Event", maxLabelLength),
  ...
  function getCurrentTarget() {
    var event = findSelectedTimelineEvent();
    var tracked = world.trackedLineage || null;

    if (world.spotlightState && world.spotlightState.active && world.spotlightEvent) {
      return copyEventTarget(world.spotlightEvent, "spotlight");
    }

    if (event) {
      return copyEventTarget(event, "timeline");
    }

    if (tracked) {
      return {
        type: "lineage",
        label: cleanText(tracked.label || "Lineage", maxLabelLength),
  ...
  function normalizeBookmark(bookmark) {
    var source = bookmark || {};
    var id = cleanText(source.id || ("B" + (world.nextBookmarkId || 1)), 24);
    var target = source.target ? clonePersistencePlainValue(source.target) : { type: "camera" };
    var camera = source.camera ? clonePersistencePlainValue(source.camera) : getCameraSnapshot();

    return {
      id: id,
      label: cleanText(source.label || getTargetLabel(target), maxLabelLength),
      note: cleanText(source.note || "", maxNoteLength),
      ...
      epoch: cleanText(source.epoch || "-", 40),
      camera: {
      ...
      target: target,
      screenshotRef: cleanText(source.screenshotRef || "", 160)
    };
  }

  function restoreBookmarks(bookmarks, nextId) {
    var source = Array.isArray(bookmarks) ? bookmarks : [];
    var restored = [];
    var highest = 0;

    for (var i = 0; i < source.length && restored.length < maxBookmarks; i++) {
      var bookmark = normalizeBookmark(source[i]);
  ...
  function createBookmark(options) {
    ...
    var target = options.target ? clonePersistencePlainValue(options.target) : getCurrentTarget();
    var id = "B" + Math.max(1, Math.round(Number(world.nextBookmarkId) || 1));
    var bookmark = normalizeBookmark({
      id: id,
      label: options.label || getTargetLabel(target),
      note: options.note || "",
      createdTick: world.tick,
      tick: world.tick,
      deepTimeYears: getDeepTimeYears(),
      epoch: world.era || "-",
    ...
    while (world.bookmarks.length > maxBookmarks) {
      world.bookmarks.pop();
    }
  ...
  function updateBookmark(id, patch) {
    ...
    if (patch && Object.prototype.hasOwnProperty.call(patch, "label")) {
      bookmark.label = cleanText(patch.label, maxLabelLength) || bookmark.label;
    }

    if (patch && Object.prototype.hasOwnProperty.call(patch, "note")) {
      bookmark.note = cleanText(patch.note, maxNoteLength);
    }
  ...
  function findMatchingEvent(target) {
    var events = Array.isArray(world.timelineEvents) ? world.timelineEvents : [];
  ...
  function getStatus(bookmark) {
    var target = bookmark && bookmark.target ? bookmark.target : {};

    if (target.type === "event") {
      return findMatchingEvent(target) ? "active" : "stale event";
    }
  ...
  function jumpToBookmark(id) {
    ...
    applyCamera(bookmark.camera);
    target = bookmark.target || {};

    if (target.type === "event") {
      event = findMatchingEvent(target) || target;
  ...
  function sync() {
    ...
    setElementHtml(bookmarkList, bookmarks.map(makeBookmarkItem).join(""));
  }
  ...
  return {
    ...
    createFromCurrentContext: createBookmark,
    update: updateBookmark,
    delete: deleteBookmark,
    get: getBookmark,
    getStatus: getStatus,
    jumpTo: jumpToBookmark,
    restore: restoreBookmarks,
    cleanText: cleanText
  };
})();
