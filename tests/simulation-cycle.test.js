...
var performanceTicks = 40;
var performanceSamples = 3;
var simulationAverageTickBudgetMs = typeof process !== "undefined" && process.platform === "win32" ? 50 : 20;
...
var averageTickSamples = [];
var averageTickMs = Infinity;
...
for (var perfSample = 0; perfSample < performanceSamples; perfSample++) {
  ...
  var sampleAverageTickMs = (performance.now() - performanceStartedAt) / performanceTicks;
  averageTickSamples.push(Number(sampleAverageTickMs.toFixed(3)));
  averageTickMs = Math.min(averageTickMs, sampleAverageTickMs);
}
...
assert.ok(
  ...
  "1400-organism simulation tick should average under " + simulationAverageTickBudgetMs + "ms, got best " + averageTickMs.toFixed(3) + "ms from samples " + averageTickSamples.join(",") + "ms"
);
...
console.log("simulation cycle checks passed", JSON.stringify({
  ...
  averageTickSamples: averageTickSamples
}));
